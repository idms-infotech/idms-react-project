...
const ChildPartProductionRepository = require("../../../../models/production/repository/childPartProductionRepository");
...
exports.create = asyncHandler(async (req, res) => {
    try {
        ...
        const itemDetails = await ChildPartProductionRepository.createDoc(createdObj);
        if (itemDetails) {
            return res.success({
...
exports.getAll = asyncHandler(async (req, res) => {
    try {
        ...
        let rows = await ChildPartProductionRepository.getAllPaginate({
            pipeline,
            project,
...
exports.getById = asyncHandler(async (req, res) => {
    try {
        let existing = await ChildPartProductionRepository.getDocById(req.params.id);
        if (!existing) {
...
exports.deleteById = asyncHandler(async (req, res) => {
    try {
        const deleteItem = await ChildPartProductionRepository.deleteDoc({_id: req.params.id});
        if (deleteItem) {
            return res.success({
...
exports.update = asyncHandler(async (req, res) => {
    try {
        let itemDetails = await ChildPartProductionRepository.getDocById(req.params.id);
        if (!itemDetails) {
            const errors = MESSAGES.apiErrorStrings.INVALID_REQUEST;
            return res.preconditionFailed(errors);
        }
        itemDetails.updatedBy = req.user.sub;
        itemDetails = await ChildPartProductionRepository.updateDoc(itemDetails, req.body);
        if (itemDetails.status == OPTIONS.defaultStatus.APPROVED) {
            ...
            let MRN = await MRNController.createDirectMRN({
                ...
                GRNDetails: await Promise.all(
                    itemDetails.childPartProductionDetails.map(async (ele, idx) => {
...
exports.getAllMasterData = asyncHandler(async (req, res) => {
    try {
        let childItemListOptions = await ProdItemRepository.filteredProdItemList([
            {
                $match: {
                    company: ObjectId(req.user.company),
...
exports.getTotalNoOfChildPartProducedPerDay = async company => {
    ...
    const rows = await ChildPartProductionRepository.filteredChildPartProdList([
        {
            $addFields: {
